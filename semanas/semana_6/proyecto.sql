...
-- ============================================
-- REPORTE 1: Totales globales del plantel
-- ============================================

SELECT
    COUNT(*)           AS total_corredores,
    ROUND(AVG(edad), 1) AS edad_promedio,
    MIN(edad)          AS corredor_mas_joven,
    MAX(edad)          AS corredor_mas_veterano,
    ROUND(AVG(peso_kg), 1) AS peso_promedio_kg
FROM corredores
WHERE activo = 1;

-- ============================================
-- REPORTE 2: Corredores por equipo
-- ============================================
...
FROM equipos e
LEFT JOIN corredores c ON c.equipo_id = e.id AND c.activo = 1
GROUP BY e.id, e.nombre
ORDER BY total_corredores DESC;

-- ============================================
-- REPORTE 3: Corredores por categoría
-- ============================================

SELECT
    cat.nombre          AS categoria,
    COUNT(c.id)         AS total_corredores,
    ROUND(AVG(c.peso_kg), 1) AS peso_promedio
FROM categorias cat
LEFT JOIN corredores c ON c.categoria_id = cat.id
GROUP BY cat.id, cat.nombre
ORDER BY total_corredores DESC;

-- ============================================
-- REPORTE 4: HAVING — equipos con más de 3 corredores
-- ============================================
